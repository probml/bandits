...
def main(config):
    ...
    #npulls, nwarmup, ntrials = 20, 2000, 10
    npulls, nwarmup, ntrials = 20, 2000, config.ntrials
    ...
    for env_name, env in environments.items():
        ...
        for model_name, model in models.items():
            for bandit_name, properties in bandits.items():
                ...
                key = PRNGKey(314)
                start = time()
                warmup_rewards, rewards_trace, opt_rewards = train(key, properties["bandit"], env, npulls, ntrials,
    ...
    #df.to_csv("bandits/results/tabular_results.csv")
    df.to_csv(config.results_filename)
